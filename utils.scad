...
module tabWithHole(tabDiam = 10, holeDiam = 5, tabLen = 0, thickness=5, centerX = false, centerY = true, centerZ = false, rotateX=0, rotateY=0, rotateZ=0) {

    rotate([rotateX, rotateY, rotateZ]) 
        translate([centerX ? -tabLen - tabDiam/2 : 0, 
                   centerY ? 0 : tabDiam/2, 
                   centerZ ? -thickness/2 : 0])
            difference() {
                union() {
                    translate([0, -tabDiam/2, 0])
                        cube([tabDiam/2 + tabLen, tabDiam, thickness]);
                    translate([tabLen+tabDiam/2, 0, 0])
                        cylinder(r = tabDiam/2, h= thickness, $fn=40);                    
                }
                translate([tabLen+tabDiam/2, 0, -1])
                    cylinder(r = holeDiam/2, h= thickness+2, $fn=40);                    
            }    
}
